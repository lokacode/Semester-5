...
# Memplot posisi awal Bumi dan Matahari
...
# Memplot garis orbit Bumi dan Matahari
...
ax.set_title('Simulasi Orbit Bumi dengan Runge-Kutta Orde 4')
ax.legend()

def update(frame):
    # Memperbarui posisi titik Bumi
    earth_dot.set_xdata(simulation.history[frame, 6])
    earth_dot.set_ydata(simulation.history[frame, 7])
    sun_dot.set_xdata(simulation.history[frame, 0])
    sun_dot.set_ydata(simulation.history[frame, 1])
    
    return earth_dot, sun_dot

ani = animation.FuncAnimation(fig, update, frames=len(simulation.history), interval=10, blit=True)
# ani.save('orbit2d.mp4', fps=30, extra_args=['-vcodec', 'libx264'], dpi=300)
plt.show()