...
num_barriers = 7
barrier_positions = np.sort(np.linspace(8.5, 11.5, num_barriers))
...
barrier_data.to_excel('barrier_data7.xlsx', index=False)
...
ax.axvspan(8.5, 11.5, color='forestgreen', alpha=0.2, lw=0, label='Area mangrove')
...
plt.tight_layout()
# ani.save('gelombang7.mp4', dpi=300, fps=30, writer='ffmpeg')
plt.show()