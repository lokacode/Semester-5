# 1. buat fungsi RungeKutta order 5
def RungeKutta(t, ht, y, n, Func):
    """
    Propagates the solution y of a system of 1st order ODEs
    y'[i] = f[i](t, y[]), i = 1..n
    from t to t+ht using the 5th order Runge-Kutta method.
...
# 2. import module yang diperlukan
from math import *
import numpy as np

# 3. tulis fungsi yang akan diintegrasikan
g = 9.81e0 # gravitational acceleration
def Func(t, u, f): # RHS of 1st order ODEs
    f[1] = u[2] # u[1] = u, u[2] = u’
    f[2] = -g/l * sin(u[1]) - k * u[2]

# 4. tentukan parameter awal
...
# 5. untuk setiap nilai u0, hitung periodenya dan simpan dalam file
...
# 6. untuk setiap nilai u0, hitung periodenya dan simpan dalam list
for i in range (len(u0)):
    t = 0e0
    u[1] = u0[i]; u[2] = du0 # initial values
    nT = 0 # number of half-periods
...
# 7. buat grafik analitik vs numerik
