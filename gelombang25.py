...
num_barriers = 25
barrier_positions = np.sort(np.linspace(1/4*x_max, 3/4*x_max, num_barriers))
...
barrier_data.to_excel('barrier_data25.xlsx', index=False)
...
ax.axvspan(1/4*x_max, 3/4*x_max, color='forestgreen', alpha=0.2, lw=0, label='Area mangrove')
...
plt.tight_layout()
ani.save('gelombang25.mp4', dpi=300, fps=30, writer='ffmpeg')
plt.show()
